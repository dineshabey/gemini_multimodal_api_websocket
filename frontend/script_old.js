...
const PROXY_URL = "ws://localhost:3320";
const PROJECT_ID = "physician-ai-chat-project";
...
geminiLiveApi.onErrorMessage = (message) => {
    ...
    showDialogWithMessage(`Error: ${message}`);
    setAppStatus("disconnected");
};

// Log connection close details
geminiLiveApi.onConnectionClosed = (event) => {
...
geminiLiveApi.onConnectionStarted = () => {
    ...
    geminiLiveApi.sendTextMessage("Hello from the client!");
    startAudioInput();
};
...
function connectBtnClick() {
    ...
    console.log("Access Token:", accessTokenInput.value);
...
function micBtnClick() {
    console.log("Mic Button Clicked: Muting microphone");
    stopAudioInput();
    micBtn.hidden = true;
...
function micOffBtnClick() {
    console.log("Mic Off Button Clicked: Unmuting microphone");
    startAudioInput();
    micBtn.hidden = false;
...
function newMicSelected() {
    console.log("New Microphone Selected:", micSelect.value);
    liveAudioInputManager.updateMicrophoneDevice(micSelect.value);
}

function disconnectBtnClick() {
    console.log("Disconnect Button Clicked: Disconnecting WebSocket");
    setAppStatus("disconnected");
...
async function getAvailableDevices(deviceType) {
    ...
    return allDevices
        ...
        .map(device => ({ id: device.deviceId, name: device.label || device.deviceId }));
}
